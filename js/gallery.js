...
function cardList({ preview, original, description }) {
...
function cardLists(items) {
  const markup = items.map(cardList).join('\n');
...
function clickItem(click) {
  const link = click.target.closest('.gallery-link');
  if (!link) return;
  click.preventDefault();
  const img = link.querySelector('.gallery-image');
  const bigPicture = img.dataset.source;
